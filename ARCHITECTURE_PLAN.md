...
- **Propagation delay queue:** Replace instant propagation with a future-event queue keyed by `tick + time_cost`. Gates schedule output updates when inputs change and handle glitches.
...
- **Verilog/SystemVerilog export:** Traverse the JSON graph to emit structural Verilog with sanitized identifiers, modules, assignments, and wires.
...
- **Assembler upgrade path:** When an ISA exceeds simple instruction patterns, swap in a small grammar/parser for robustness.
